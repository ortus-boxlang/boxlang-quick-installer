...
# Run a specific shell test suite
cd tests && ./run.sh --single helpers_test

# List available test suites
cd tests && ./run.sh --list

# Run all PowerShell tests on Windows or PowerShell 7+
pwsh -File tests/powershell/run.ps1

# Run one PowerShell test suite
pwsh -File tests/powershell/run.ps1 -TestName Helpers
```

Shell test suites in `tests/specs/`:

- `alpine_sh_install_test.sh`: Alpine installation behavior
- `bvm_use_test.sh`: BVM version selection behavior
- `helpers_test.sh`: Helper functions
- `install_boxlang_test.sh`: Single-version installer behavior
- `install_bx_module_test.sh`: ForgeBox module installer behavior
- `java_version_test.sh`: Java version detection
- `local_artifact_install_test.sh`: Local artifact installation
- `preflight_check_test.sh`: Pre-flight dependency checks

The shell runner discovers these files automatically, so new suites only need to
follow the `*_test.sh` naming convention. PowerShell tests are in `tests/powershell/`
and are run by `tests/powershell/run.ps1`.
...
- Use local artifacts and `--non-interactive` when testing without network access or prompts
