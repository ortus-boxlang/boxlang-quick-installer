...
Invoke-Test 'installs local JAR artifacts and a script directory without downloads' {
    ...
    try {
		...
		Assert-True ($result.Output -notmatch 'Forcing reinstallation of BoxLang') 'Fresh forced install should not be reported as a reinstallation'
		Assert-True ($result.Output -notmatch 'Removing previous BoxLang installation') 'Fresh forced install should not remove a previous installation'
		Assert-True ($result.Output -notmatch 'Previous installation removed successfully') 'Fresh forced install should not report a removed installation'
